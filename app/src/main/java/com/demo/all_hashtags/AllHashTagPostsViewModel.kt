...
class AllHashTagPostsViewModel(
    ...
    var hashTag = MutableLiveData(HashTag())
    var hashTagPosts = MutableLiveData<ArrayList<PostAssociated>>(ArrayList())
    private var followHashTagRequest = MutableLiveData(FollowHashTagRequest())
    private var getPostParams = MutableLiveData(QueryParams())
    ...
    fun followHashTag() {
        ...
        hashTagRepository.followHashTag(followHashTagRequest.value!!,
            onResult = { isSuccess: Boolean, response: BaseResponse ->
                ...
                } else response.error?.message?.let { toastMessage.value = it }
            })
    }
