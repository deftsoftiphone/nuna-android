...
uniform sampler2D glfx_BACKGROUND, tex;
in vec2 var_bg_uv;
in float var_factor;
...
uniform sampler2D selection_tex, lookupTexEyes, lookupTexTeeth;
...
void main()
{
	...
	vec4 makeup = texture( tex, var_uv );
	F = vec4( mix( res.xyz, makeup.xyz, makeup.w*var_factor ), 1. );
}
