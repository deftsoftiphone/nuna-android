...
in vec4 script_data1;
in vec2 var_bg_uv;
uniform sampler2D glfx_BG_MASK;
uniform sampler2D tex_emptyBG;
...
void main()
{
    ...
    if (script_data1.x > 0.5) {
		mask = texture( glfx_BG_MASK, var_bg_uv ).x;
	} else {
		mask = texture( tex_emptyBG, var_bg_uv ).x;
	}

    frag_color = vec4(l2g(color),opacity);

    // frag_color = vec4(mask, mask, mask, mask);
	frag_color.a = mask;
}